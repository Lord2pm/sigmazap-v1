...
def responder_mensagem():
    ...
    # Pegar a senha, fazer login e caso o usuário se logue, exibir os negócios
        ...
        print(session["status_login"])
        if session["status_login"]:
            resp.message(
                "ID: 30 => CONSULTORIA\nID: 32 => CONSULTORIA - Eduardo\nID: 31 => sigma calendario\n\nEscolha o banco de dados pelo seu ID. Exemplo: 30"
            )
            session["i"] += 1
        else:
    ...
    # Entra na parte de serviços
        ...
        if session["servico_escolhido"] == 1:
            if session["i_servico"] == 0:
                ...
                session["ss_data"]["MAQ_COD"] = (
                    msg if msg.lower() != "v" else session["ss_data"]["MAQ_COD"]
                )
                ...
                session["ss_data"]["TAG"] = (
                    msg if msg.lower() != "v" else session["ss_data"]["TAG"]
                )
        ...
        # Emissão de OS
            ...
            if session["i_servico"] == 0:
                ...
                session["os_data"]["MAQ_COD"] = (
                    msg if msg.lower() != "v" else session["os_data"]["MAQ_COD"]
                )
                resposta = validar_cod_desc_maquina(msg) if msg.lower() != "v" else ""
                ...
                session["os_data"]["TAG"] = (
                    msg if msg.lower() != "v" else session["os_data"]["TAG"]
                )
                resposta = validar_cod_desc_tag(msg) if msg.lower() != "v" else ""
                ...
                try:
                    session["os_data"]["OS_AFETAPR"] = (
                        {1: 1, 2: 0}[int(msg)]
                        if msg.lower() != "v"
                        else session["os_data"]["OS_AFETAPR"]
                    )
        ...
        # Consulta de OS
            ...
            lista_de_ss = consultar_todas_ss(
                session["email"], session["senha"], session["db_id"], "P"
            )
            resp.message(f"Lista de SSs pendentes\n\n{lista_de_ss}")
        elif session["servico_escolhido"] == 6:
            lista_de_ss = consultar_todas_ss(
                session["email"], session["senha"], session["db_id"], "A"
            )
            resp.message(f"Lista de SSs concluidas\n\n{lista_de_ss}")
        elif session["servico_escolhido"] == 7:
            lista_de_ss = consultar_todas_ss(
                session["email"], session["senha"], session["db_id"], "N"
            )
